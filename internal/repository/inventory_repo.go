...
func (r *InventoryRepository) IsEventProcessed(ctx context.Context, tx *sqlx.Tx, eventID string) (bool, error) {
	var exists bool
	...
	if tx != nil {
		...
		if err != nil {
			log.Error().Err(err).Str("event_id", eventID).Msg("Failed to check if event is processed")
...
// MarkEventProcessed marks an event as processed to prevent duplicate processing
func (r *InventoryRepository) MarkEventProcessed(ctx context.Context, tx *sqlx.Tx, event *models.InventoryEvent) error {
	query := `INSERT INTO processed_events (event_id, event_type, sku, processed_at) 
			  VALUES ($1, $2, $3, NOW()) 
			  ON CONFLICT (event_id) DO NOTHING`

	if tx != nil {
		_, err := tx.ExecContext(ctx, query, event.EventID, event.EventType, event.SKU)
		if err != nil {
			log.Error().Err(err).
		...
		_, err := r.db.ExecContext(ctx, query, event.EventID, event.EventType, event.SKU)
		if err != nil {
			log.Error().Err(err).
				Str("event_id", event.EventID).
				Str("event_type", event.EventType).
...
func (r *InventoryRepository) CleanupOldProcessedEvents(ctx context.Context, olderThan time.Duration) error {
	query := `DELETE FROM processed_events WHERE processed_at < NOW() - INTERVAL '%d hours'`
	hours := int(olderThan.Hours())

	result, err := r.db.ExecContext(ctx, fmt.Sprintf(query, hours))
	if err != nil {
	...
	log.Info().Int64("rows_deleted", rowsAffected).Msg("Cleaned up old processed events")
